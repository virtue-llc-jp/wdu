...
| `current_usage_bytes` | `u64` | 最後に正常完了した再計算での配下全体の論理サイズ |
...
現在の使用量は通常ファイルの論理サイズ（`metadata.len()`）の合計です。シンボリック
リンク、ディレクトリエントリ自身の inode サイズ、socket・device・FIFO は合計せず、
ハードリンクはパスごとに 1 回ずつ数えます。
